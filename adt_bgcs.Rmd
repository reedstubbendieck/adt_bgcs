---
title: "adt_bgcs"
author: "Reed M. Stubbendieck & Cameron R. Currie"
...
eHOMD_BGCs <- eHOMD_BGCs %>%
  ...
  filter(!(cluster == "cluster035" | cluster == "cluster666" | cluster == "cluster1400")) %>%
  # mutate hybrid clusters denoted by a hyphen into their own cluster type
  mutate(type = ifelse(grepl('-',type), "hybrid", type)) %>%
  ...
  # add a new column for cluster_type and collapse subtypes into cluster_type
  mutate(cluster_type = ifelse(
    type == "bacteriocin" | type == "sactipeptide" | type == "thiopeptide" | type == "lantipeptide" | type == "lassopeptide" | type == "microcin" | type == "cyanobactin" | type == "linaridin" | type == "proteusin" | type == "glycocin", "RiPP", 
    ifelse(type == "t1pks" | type == "t2pks" | type == "t3pks" | type == "transatpks" | type == "resorcinol" | type == "otherks", "pks", 
    ifelse(type == "ladderane" | type == "acyl_amino_acids" | type == "blactam" | type == "phenazine", "other", type))
    )) %>%
  select(cluster, type, cluster_type, contig_edge, size, SEQFID, genus_type, genus, species, strain)
...
eHOMD_BGCs_by_strain <- eHOMD_BGCs_subset %>%
  group_by(genus, species, strain, genus_type) %>%
  summarize(Count = n())
...
eHOMD_genome_info <- eHOMD_genome_info %>%
  select(SEQFID, Contigs, Size_bp, count, genus_type) %>%
...
# remove intermediary objects that are no longer needed for subsequent analysis or figure generation
rm(eHOMD_pks_nrps_cluster_info)
```
...
clusters_meta <- clusters_meta %>%
  select(cluster_type, genus_type)
clusters_meta <- as.data.frame(clusters_meta)
colnames(clusters_meta) <- c("Type", "Genus")

##### reorder the annotations
clusters_meta <- clusters_meta[,c(2,1)]
...
Figure_1_A <- ggplot() +
  ...
  scale_x_discrete(limits=rev(c("RiPP", "nrps", "terpene", "arylpolyene", "pks", "siderophore", "hserlactone", "ectoine", "phosphonate", "butyrolactone", "hybrid", "other")),
                              labels=rev(c("RiPP", "NRPS", "Terpene", "Aryl Polyene", "PKS", "Siderophore", "HSL", "Ectoine", "Phosphonate", "Butyrolactone", "Hybrid", "Other")))
...
Figure_1_C <- ggplot() +
  geom_jitter(data = eHOMD_BGCs_by_strain, aes(y=Count, x=genus_type, fill=genus_type), shape = 21, col="black", stroke=0.1, height = 0.1, size=1.5) +
...
Figure_1_D <- ggplot() +
  geom_jitter(data=eHOMD_genome_info, aes(y=Size_bp, x=count, fill=genus_type), shape = 21, col="black", stroke=0.1, width = 0.25, size=1.5) +
  ylab("Genome Size (Mb)") +
...
# Figure S2
## jitter plot of the log10 relative abundance of select genera in the buccal mucosa, tongue dorsum, external naris, and gingiva
### cleaned up in Inkscape to italicized the facet labels

Figure_S2 <- ggplot() +
  geom_jitter(data = hmp_adt_metaphlan_genus_subset, aes(y=log_ra, x=site, fill=site), shape = 21, col="black", stroke=0.1) +
  xlab("Site") +
  ...
  facet_wrap(~genus_type, ncol = 6) + 
  stat_summary(data = hmp_adt_metaphlan_genus_subset, aes(y=log_ra, x=site, fill=site), fun = "median", fun.min = "median", fun.max= "median", size=0.3, geom = "crossbar", color = "black", show.legend = FALSE)

ggsave("./figures/Figure_S2.pdf", Figure_S2, units=c("in"), width=8, height=5)
