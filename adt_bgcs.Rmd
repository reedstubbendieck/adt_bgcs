...
### bioactivity data
actinomyces_bioassay_data <- read_delim("rawData/actinomyces_bioassays_anaerobic.tsv", 
                                                  "\t", escape_double = FALSE, trim_ws = TRUE)
...
actinomyces_bioassay_data <- actinomyces_bioassay_data %>%
  group_by(strain, target) %>%
  summarize(mean_score = mean(score))

actinomyces_bioassay_data$target <- factor(actinomyces_bioassay_data$target, 
...
```{r Figure 9A, include=FALSE, echo=FALSE}
Figure_8A <- ggplot() +
  geom_tile(data=actinomyces_bioassay_data, aes(x=strain, y=target, fill=mean_score)) +
  adt_bgc_gg_theme +
